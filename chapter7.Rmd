...
\[
\hat{\beta} = \argmin{\beta} \sum_{i=1}^n (x_i^\top\beta - y_i)^2 = (X^\top X)^{-1} X^\top Y
\]
...
\[
\hat{\beta} = \argmin{\beta} \sum_{i=1}^n w_i( x_i^\top\beta - y_i)^2 = (X^\top WX)^{-1}X^\top WY
\]
...
* So what does WLS do? 
    ...
    1. It __is__ optimal, in the sense of G-M, if $\Var{\epsilon_i}=\sigma^2(x_i)$.
...
\[
\hat{c} = \argmin{c} \sum_{j=1}^n \sum_{i=1}^n w_{ij}(c_j - y_i)^2 \quad w_{ij} = \frac{K((x_i-x_j)/h)}{\sum_{i=1}^n K((x_i-x_j)/h)}
\]
...
* Some things don't change. 
    ...
    1. We still have that $\Expect{\hat{\beta}} = \beta$. That is OLS __is__ still unbiased.
...
```{r, fig.height = 4, fig.align='center', fig.width=8, results='hide', message=FALSE,echo=FALSE}
set.seed(1234)
trueFunction <- function(x) sin(x) + 1/sqrt(x)
x = runif(100, 0, 2*pi)
x = seq(min(x), max(x), length.out=100)
...
* We __know__ that $\Expect{\hat{e}\given X=x} = 0$.

* We __know__ that $\Expect{\hat{e}^2 \given X=x} = \sigma^2(x)$.
...
plot(jobInt$x,origBetas$log2resids,bty='n',las=1,pch=19,col=4,xlab='x',ylab='log sq-resids')
xx=sort.int(jobInt$x,index.return = TRUE)$ix
lines(jobInt$x[xx],log(origBetas$weights)[xx],lwd=2,col=2)
```
...
* This chapter is another way of fitting "smooth" curves for regression: splines.

* Splines are very nice, and they turn out to be __linear smoothers__: we can write 

\[
\hat{Y} = WY.
\]

* Splines are given by solving

\[
\hat{\mu}_{\lambda} = \argmin{m} \sum_{i=1}^n (y_i - m(x_i))^2 + \lambda \int dx (m''(x))^2.
\]

* The first half you know: it's the MSE

* The second half __penalizes__ the second derivative of $m$.

* Second derivatives measure the curvature of a function, the smaller, the smoother.

* With Kernels, we used the bandwidth to control the smoothness, here $\lambda$ controls it directly.

## Some notes

* Of course we choose $\lambda$ with CV (or GCV)

* Sorry, there is not a good reason (that I know) to prefer splines over Kernels or vice-versa.

* Skip 8.6 (or read it very lightly, and don't ask me questions about it)
