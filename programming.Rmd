...
value ($\hat{y}$) and a residual ($y-\hat{y}$).  These are vectors, and
...
```{r test, cache=TRUE, echo=FALSE, results='hide'}
library(np)
...
```{r, eval=FALSE, cache=TRUE, autodep=TRUE}
library(np)
mob.np <- npreg(formula=formula(mob.lm2), data=mob, tol=1e-2, ftol=1e-2)
```
