...
1. Get some data $(y_1,x_1),\ldots,(y_n,x_n)$, figure out the __link function__ $g$, and calculate $g'$ and $\Var{Y\given X=x}$. Now give some initial guesses for $\beta$ (say $\beta=\mathbf{0}$)

2. Iterate until convergence:
    
    a. Calculate $\eta(x_i) = \beta^\top x_i$ and $\hat{\mu}(x_i) = g(\eta(x_i))$.
...
c(b0,b)
coef(glm(y.logit~x))
irwls(y.logit, cbind(1,x), invlink=ilogit, linkPrime=function(m) 1/(m*(1-m)), V=function(m) m*(1-m))
```
...
library(ggplot2) # might need to install this package
...
```{r}
require(nnet)
full = multinom(apply~pared+public+gpa, data=dat, trace=FALSE) # This fits the unconstrained model (not ordinal)
...
<img src=pm.png width=1200 height=800>
...
```{r}
require(plyr)
sugary = dlply(soda, .(Site), glm, formula=Regular~DofW+cont+offset(log(CosTot)), family='quasipoisson')
zerocal = dlply(soda, .(Site), glm, formula=ZeroCal~DofW+cont+offset(log(CosTot)), family='quasipoisson')
...
library(reshape2)
df = rbind.fill(melt(((exp(sapply(sugary,coef)))-1)*100),melt(((exp(sapply(zerocal,coef)))-1)*100))
